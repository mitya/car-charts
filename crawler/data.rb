...
Drives = { "полный" => "4WD", "передний" => "FWD", "задний" => "RWD" }
ShortFuelSigns = { "бензин" => "i", "дизель" => "d" }
ShortTransmissionKeys = {"AMT" => "RT", "AT" => "AT", "CVT" => "VT", "MT" => "MT"}

AllKeys = %w(body version_key transmission drive engine_vol fuel power model_key
...
RejectedKeys = %w(
  ...
  model_title body_title body_type price engine_title engine_spec category).to_set

def rowIndexForKey(key)
  AllKeys.index(key)
end
...
VendorKeysToTitles = VendorTitleToKeys.invert

ModelEnglishTitleFixes = {
...
FloatSpecs = %w(acceleration_0_100_kmh bore stroke compression engine_vol).to_set
IntegerSpecs = %w(top_speed engine_volume cylinder_count valves_per_cylinder gears max_power max_power_kw max_torque max_power_revs max_torque_revs
  length width height ground_clearance front_tire_rut rear_tire_rut wheelbase luggage_capacity tank_capacity kerbweight gross_mass doors).to_set
EnumSpecs = %w(cylinder_placement injection engine_placement drive_config front_suspension rear_suspension front_brakes rear_brakes fuel 
  fuel_rating compressor transmission).to_set
EnumSpecsSelected = EnumSpecs
...
SpecValueTranslations = {
 ...
 "front_suspension"=> SpecValueSuspensions,
 "rear_suspension"=> SpecValueSuspensions,
 "front_brakes"=> SpecValueBrakes,
 "rear_brakes"=> SpecValueBrakes,
...
BrandedModelNameFixes = {
  "land_rover--range_rover"           => "Range Rover",
  "land_rover--range_rover_evoque"    => "Range Rover Evoque",
  "land_rover--range_rover_sport"     => "Range Rover Sport",  
}

RusCountryNamesToCodes = {
  "Австрия" => "AT",
  "Бельгия" => "BE",
  "Бразилия" => "BR",
  "Великобритания" => "GB",
...
RusMonths = %w(Январь Февраль Март Апрель Май Июнь Июль Август Сентябрь Октябрь Ноябрь Декабрь)
